...
3. Enter a narrow, source-aware instruction.
...
- CSS: `In style.css, inside .button-link, change background from var(--accent) to #0b3d91. Do not change anything else.`
...
Exact current values reduce rejected patches. The fixture title is `Harbor Light Studio`, not `Home` (which is navigation text).
...
| `clarification_required` | Manager needs a more specific request; no staging or specialist ran. |
