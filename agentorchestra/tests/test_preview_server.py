...
def test_preview_server_serves_temp_index(tmp_path):
    ...
    with preview_server(tmp_path) as base_url, urlopen(
...
def test_preview_server_rejects_invalid_roots(tmp_path):
    with pytest.raises(ValueError), preview_server(tmp_path / "missing"):
        pass

    file_root = tmp_path / "file.txt"
    file_root.write_text("not a directory", encoding="utf-8")
    with pytest.raises(ValueError), preview_server(file_root):
        pass
