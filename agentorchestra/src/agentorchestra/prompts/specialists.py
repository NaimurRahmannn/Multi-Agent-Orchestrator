...
3. Before each patch attempt, read a bounded range from the same staged file that fully contains the intended target. If an applied patch changed that range, reread it before another patch.
...
9. A patch is applied only when propose_patch returns status applied. Never infer success from your intended arguments, task summary, or a rejected tool result.
10. Return completed only after at least one propose_patch result has status applied and the applied evidence satisfies the assignment. If every patch attempt was rejected, completed is forbidden; return blocked with the actual remaining issue.
...
Completed is valid only when at least one propose_patch result returned status applied. If all attempts were rejected, return blocked rather than claiming completion.
""".strip()
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "Follow the verbatim exact-patch protocol: bounded read, old_text copied from its content field, minimal new_text, then completion only after status applied.",
        ]
    )
