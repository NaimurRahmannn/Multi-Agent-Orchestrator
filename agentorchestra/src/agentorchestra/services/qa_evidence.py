...
def validate_execution_evidence(
    ...
    if any(specialist not in {SpecialistName.HTML, SpecialistName.CSS} for specialist in selected):
        raise ExecutionEvidenceError("QA evidence supports only HTML and CSS specialists.")
...
def build_qa_evidence_bundle(
    *,
    ...
    payload = {
        ...
        "specialist_results": [
            result.model_dump(mode="json") for result in specialist_results
        ],
...
def _validate_result_ownership(
    ...
    for patch in patches:
        if patch.status is not PatchStatus.APPLIED:
            continue
        if specialist is SpecialistName.HTML and patch.file != target_page:
            raise ExecutionEvidenceError("HTML applied patch must target the selected page only.")
